...
- (UIButton *)skipButton {
    if (!_skipButton) {
        ...
        [_skipButton setTitleColor:[UIColor yellowColor] forState:UIControlStateNormal];
...
- (void)buildFooterView {
    ...
    if (self.skipButton && !self.skipButton.hidden) {
        ...
        if(self.skipButtonY == EA_EMPTY_PROPERTY) {
            [self.footerConstraints addObject:[NSLayoutConstraint constraintWithItem:self.pageControl attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self.skipButton attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:-30.0]];
        } else {
